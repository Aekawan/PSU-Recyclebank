    ...
    public function find_today()
    {
      ...
      foreach ($sales as $index => $sale) {
       ...
       if(count($profit) > 0){
         ...
         Session::flash('msg', 'กำไร/ขาดทุนวันนี้');
         return view('admin.profit.index',['finddate' => date("d/m/Y"),'saledate' => $newdate, 'sales' => $sales, 'profit' => $profit,'sumprofit' => $sumprofit, 'sumloss' => $sumloss,'totalsale'=>$totalsale,'totalbuy'=>$totalbuy,'totalprofit'=>$totalprofit,'totalkilo'=>$totalkilo,'totalloss'=>$totalloss]);
       } else {
         list($day,$month,$year) = explode("/", date("d/m/Y"));
         $newdate = $day."/".$month."/".$year;
         return view('admin.profit.index',['finddate' => $newdate,'profit' => $profit, 'sales' => $sales,'totalsale'=>$totalsale,'totalbuy'=>$totalbuy,'totalprofit'=>$totalprofit,'totalloss'=>$totalloss]);
       }
    ...
    public function find_date($saledate)
    {
     ...
     /*
      $total =  DB::table('sales')
                      ->select(DB::raw('sum(sales.purchase_price) as sum_purchase_price'),DB::raw('sum(sales.sale_price) as sum_sale_price'),DB::raw('sum(sales.unit) as sum_sale_unit'),DB::raw('sum(sales.profit) as sum_sale_profit'),DB::raw('sum(sales.loss) as sum_sale_loss'))
                      ->where('sales.created_at','LIKE',$datefind)
                      ->get();
     */
       if(count($profit) > 0){
         if ($profit[0]->sum_purchase_price < $profit[0]->sum_sale_price) {
         ...
         Session::flash('msg', 'กำไร/ขาดทุนวันที่ '.$newdate);
         return view('admin.profit.index',['finddate' => $query,'saledate' => $newdate, 'sales' => $sales, 'profit' => $profit,'sumprofit' => $sumprofit, 'sumloss' => $sumloss,'totalsale'=>$totalsale,'totalbuy'=>$totalbuy,'totalprofit'=>$totalprofit,'totalloss'=>$totalloss,'totalkilo'=>$totalkilo]);
       } else {
         list($day,$month,$year) = explode("/", date("d/m/Y"));
         $newyear = intval($year);
         $newdate = $day."/".$month."/".$year;
         return view('admin.profit.index',['finddate' => $newdate,'profit' => $profit, 'sales' => $sales,'totalsale'=>$totalsale,'totalbuy'=>$totalbuy,'totalprofit'=>$totalprofit,'totalloss'=>$totalloss,'totalkilo'=>$totalkilo]);
       }
       } else {
         list($day,$month,$year) = explode("/", date("d/m/Y"));
         $newyear = intval($year);
         $newdate = $day."/".$month."/".$year;
         return view('admin.profit.index',['profit' => null,'finddate' => $newdate]);
     }
    }
    ...
    public function find_month($month,$year)
    {
      ...
      foreach ($sales as $index => $sale) {
       ...
       if(count($profit) > 0){
         ...
         Session::flash('msg', 'กำไร/ขาดทุน เดือน'.$month.' '.'ปี'.' '.$year);
         return view('admin.profit.index',['finddate' => null,'saledate' => null, 'sales' => $sales, 'profit' => $profit,'sumprofit' => $sumprofit, 'sumloss' => $sumloss,'totalsale'=>$totalsale,'totalbuy'=>$totalbuy,'totalprofit'=>$totalprofit,'totalloss'=>$totalloss,'totalkilo'=>$totalkilo]);
       } else {
         list($day,$month,$year) = explode("/", date("d/m/Y"));
         $newyear = intval($year);
         $newdate = $day."/".$month."/".$year;
         return view('admin.profit.index',['finddate' => null,'profit' => $profit, 'sales' => $sales,'totalsale'=>$totalsale,'totalbuy'=>$totalbuy,'totalprofit'=>$totalprofit,'totalloss'=>$totalloss,'totalkilo'=>$totalkilo]);
       }
    }
    ...
    public function find_year($year)
    {
      ...
      foreach ($sales as $index => $sale) {
       ...
       if(count($profit) > 0){
         ...
         Session::flash('msg', 'กำไร/ขาดทุน ปี'.' '.$year);
         return view('admin.profit.index',['finddate' => $year,'saledate' => $year, 'sales' => $sales, 'profit' => $profit,'sumprofit' => $sumprofit, 'sumloss' => $sumloss,'totalsale'=>$totalsale,'totalbuy'=>$totalbuy,'totalprofit'=>$totalprofit,'totalloss'=>$totalloss,'totalkilo'=>$totalkilo]);
       } else {
         list($day,$month,$year) = explode("/", date("d/m/Y"));
         $newyear = intval($year);
         $newdate = $day."/".$month."/".$year;
         return view('admin.profit.index',['finddate' => $month+' '+$year,'profit' => $profit, 'sales' => $sales,'totalsale'=>$totalsale,'totalbuy'=>$totalbuy,'totalprofit'=>$totalprofit,'totalloss'=>$totalloss,'totalkilo'=>$totalkilo]);
       }
    }
      ...
      foreach ($sales as $index => $sale) {
       ...
       if(count($profit) > 0){
         if ($profit[0]->sum_purchase_price < $profit[0]->sum_sale_price) {
          ...
          if($msg == '7day'){
            Session::flash('msg', 'กำไร/ขาดทุนย้อนหลัง 7 วัน');
          } else if($msg == 'one_month'){
            Session::flash('msg', 'กำไร/ขาดทุนย้อนหลัง 1 เดือน');
          } else if($msg == 'three_month'){
            Session::flash('msg', 'กำไร/ขาดทุนย้อนหลัง 3 เดือน');
          } else if($msg == 'one_year'){
            Session::flash('msg', 'กำไร/ขาดทุนย้อนหลัง 1 ปี');
          }
         return view('admin.profit.index',['finddate' => null,'saledate' => null, 'sales' => $sales, 'profit' => $profit,'sumprofit' => $sumprofit, 'sumloss' => $sumloss,'totalsale'=>$totalsale,'totalbuy'=>$totalbuy,'totalprofit'=>$totalprofit,'totalloss'=>$totalloss,'totalkilo'=>$totalkilo]);
       } else {
         list($day,$month,$year) = explode("/", date("d/m/Y"));
    ...
    public function find_custom($start_date,$end_date)
    {
      ...
      foreach ($sales as $index => $sale) {
       ...
       if(count($profit) > 0){
         ...
         Session::flash('msg', 'กำไร/ขาดทุน จากวันที่ '.$start_date.' '.'ถึงวันที่'.' '.$end_date);
         return view('admin.profit.index',['finddate' => null,'saledate' => null, 'sales' => $sales, 'profit' => $profit,'sumprofit' => $sumprofit, 'sumloss' => $sumloss,'totalsale'=>$totalsale,'totalbuy'=>$totalbuy,'totalprofit'=>$totalprofit,'totalloss'=>$totalloss,'totalkilo'=>$totalkilo]);
       } else {
         list($day,$month,$year) = explode("/", date("d/m/Y"));
         $newyear = intval($year);
